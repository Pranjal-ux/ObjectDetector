...
Adapted for Render Docker deployment.
"""
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173",
        "http://127.0.0.1:5173",
        "https://vizo-object-detector.vercel.app",
    ],
...
SUPPORTED_MODELS = [
    {
        "id": "yolo26n.pt",
        "name": "YOLO26 Nano (Fastest)",
        "description": "Lightweight nano model",
    },
    {
        "id": "yolo26s.pt",
        "name": "YOLO26 Small (Balanced)",
        "description": "Balanced speed and accuracy",
    },
    {
        "id": "yolo26m.pt",
        "name": "YOLO26 Medium (Accurate)",
        "description": "Higher precision model",
    },
    {
        "id": "yolo26l.pt",
        "name": "YOLO26 Large (High Precision)",
        "description": "High accuracy (GPU recommended)",
    },
    {
        "id": "yolo26x.pt",
        "name": "YOLO26 Extra Large (Max Precision)",
        "description": "Maximum accuracy",
    },
    {
        "id": "yolo11n.pt",
        "name": "YOLO11 Nano",
        "description": "Standard YOLO11 lightweight",
    },
]
...
def list_classes():
    ...
    classes_list = [
        {"id": cid, "name": name}
        for cid, name in det.class_names.items()
    ]
...
def update_config(req: ConfigUpdateRequest):
    ...
    if (
        req.model_path is not None
        and req.model_path != app_config.model_path
    ):
    ...
    if req.target_classes is not None:
        app_config.target_classes = (
            req.target_classes
            if len(req.target_classes) > 0
            else None
        )
    ...
    if reload_model:
        ...
        detector.target_class_ids = (
            detector._resolve_target_classes(
                app_config.target_classes
            )
        )
    ...
    for idx, box in enumerate(boxes):
        ...
        t_id = (
            ...
            if (
                ...
                and idx < len(track_ids)
                and track_ids[idx] is not None
            )
            else None
        )
...
async def detect_image(
    ...
    try:
        ...
        if img is None:
            raise HTTPException(
                status_code=400,
                detail="Invalid or corrupt image format",
            )

        return JSONResponse(
            content=process_image_frame(img)
        )
...
def capture_screenshot(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Screenshot save failed: {str(e)}",
        )
